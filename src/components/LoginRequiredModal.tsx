...
interface LoginRequiredModalProps {
  onClose: () => void;
  onLogin: () => void;
  /** Optional — defaults to the original upload-specific message so
   *  existing callers (Header.tsx's upload flow) are unaffected. Pass a
   *  custom message for other gated actions (tipping, commenting, etc.)
   *  so the prompt actually explains what the person was trying to do. */
  message?: string;
}

export function LoginRequiredModal({ onClose, onLogin, message }: LoginRequiredModalProps) {
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50">
      <div className="bg-[#181818] rounded-2xl p-8 w-[90%] max-w-sm shadow-lg border border-gray-700 text-center">
        ...
        <p className="text-gray-300 mb-6">
          {message || "Please sign in to upload videos to your AirStream account."}
        </p>
